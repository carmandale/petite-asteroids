...
final class CharacterMovementSystem: System {
    ...
    required init(scene: RealityKit.Scene) {
        ...
        scene.subscribe(to: ComponentEvents.DidAdd.self,
                        ...
                        didAddCharacterMovementComponent).store(in: &subscriptions)
        scene.subscribe(to: CollisionEvents.Began.self, componentType: CharacterMovementComponent.self, onCollisionBegan).store(in: &subscriptions)
        scene.subscribe(to: CollisionEvents.Updated.self,
                        componentType: CharacterMovementComponent.self,
                        onCollisionUpdated).store(in: &subscriptions)
        scene.subscribe(to: CollisionEvents.Ended.self, componentType: CharacterMovementComponent.self, onCollisionEnded).store(in: &subscriptions)
        scene.subscribe(to: PhysicsSimulationEvents.WillSimulate.self, onPhysicsSimulationWillSimulate).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidAdd.self,
                        componentType: NotificationComponent.self,
                        onDidAddNotificationComponent).store(in: &subscriptions)
    }
