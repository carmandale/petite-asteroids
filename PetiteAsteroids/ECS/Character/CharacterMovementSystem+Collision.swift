...
extension CharacterMovementSystem {
    ...
    func onCollisionBegan(event: CollisionEvents.Began) {
        ...
        if event.entityB.components.has(PlatformAnimationMarkerComponent.self) {
            event.entityA.components[CharacterMovementComponent.self]?.currentPlatformModelEntity = event.entityB
        ...
        if let collisionClassification = characterMovement.collisionClassificationByEntity[event.entityB] {
            ...
            if isBounce {
                ...
                if let currentPlatform = characterMovement.currentPlatformModelEntity, isImpulseUp {
                    ...
                    var animationConmponent = PlatformOffsetAnimationComponent(offsetY: .leastNormalMagnitude * -1, velocity: -clampedImpulse / 90)
                    ...
                    if let existingAnimation = currentPlatform.components[PlatformOffsetAnimationComponent.self] {
                        animationConmponent.velocity += existingAnimation.velocity
                        animationConmponent.offsetY = existingAnimation.offsetY
                    }
                    currentPlatform.components.set(animationConmponent)
                }
            }
    ...
    func onCollisionEnded(event: CollisionEvents.Ended) {
        ...
        if event.entityB == characterMovement.currentPlatformModelEntity {
            event.entityA.components[CharacterMovementComponent.self]?.currentPlatformModelEntity = nil
        }
    }
