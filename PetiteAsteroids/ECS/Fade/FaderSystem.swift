...
class FaderSystem: System {
    ...
    required init(scene: Scene) {
        scene.subscribe(to: ComponentEvents.DidAdd.self,
                        componentType: ModelFaderMarkerComponent.self,
                        onDidAddModelFaderMarker).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidAdd.self, componentType: OpacityFaderComponent.self, onDidAddOpacityFader).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidAdd.self,
                        componentType: DirectionalLightFaderComponent.self,
                        onDidAddDirectionalLightFader).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidAdd.self, componentType: IBLFaderComponent.self, onDidAddIBLFader).store(in: &subscriptions)
    }
