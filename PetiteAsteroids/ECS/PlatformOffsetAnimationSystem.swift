...
final class PlatformOffsetAnimationSystem: System {
    ...
    init(scene: Scene) {
        scene.subscribe(to: ComponentEvents.DidAdd.self,
                        componentType: PlatformAnimationMarkerComponent.self,
                        onDidAddPlatformAnimationMarkerComponent).store(in: &subscriptions)
    }
    ...
    func update(context: SceneUpdateContext) {
        ...
        for entity in entities {
            ...
            // Use the offset to set the shader parameter and animate the platform.
