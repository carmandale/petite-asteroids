...
final class RockCollectorSystem: System {
    ...
    required init (scene: RealityKit.Scene) {
        scene.subscribe(to: CollisionEvents.Began.self, onCollisionBegan).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidAdd.self, componentType: RockCollectorComponent.self, onDidAddRockCollector).store(in: &subscriptions)
        scene.subscribe(to: ComponentEvents.DidChange.self, componentType: GamePlayStateComponent.self, onGameStateChanged).store(in: &subscriptions)
