...
class SpeechBubbleSystem: System {
    ...
    required init(scene: RealityKit.Scene) {
        ...
        scene.subscribe(to: CollisionEvents.Began.self,
                        componentType: SpeechBubbleTriggerComponent.self,
                        onCollisionBegan).store(in: &subscriptions)
    }
    ...
    func onDidChangeGamePlayState(event: ComponentEvents.DidChange) {
        ...
        if gamePlayState == .introAnimation,
           var (speechBubbleEntity, speechBubbleComponent) = event.entity.first(withComponent: SpeechBubbleComponent.self) {
            speechBubbleComponent.scale = 0
            speechBubbleComponent.timer = 0
            speechBubbleComponent.isEnabled = false
            speechBubbleEntity.components[SpeechBubbleComponent.self] = speechBubbleComponent
        }
    ...
    func update(context: SceneUpdateContext) {
        ...
        for speechBubble in context.entities(matching: query, updatingSystemWhen: .rendering) {
            ...
            var offset = speechBubbleComponent.offset * (speechBubbleComponent.isDown ? 1 : -1)
