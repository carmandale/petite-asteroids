...
extension AppModel {
    func loadGameAssets() async {
        ...
        try? await Task.sleep(nanoseconds: UInt64(GameSettings.menuAppearDelay * Float(NSEC_PER_SEC)))
    ...
    private func prepareInputVisualizer(inputVisualizer: Entity) {
        ...
        handInputVisualizer.components.set(HandInputVisualizerComponent(character: character,
        ...
        handInputVisualizerInsidePortal.components[HandInputVisualizerComponent.self]?.directionIndicator = directionIndicatorCopy
        handInputVisualizerInsidePortal.components[HandInputVisualizerComponent.self]?.jumpIndicator = jumpIndicatorCopy
    }
    
    private func prepareLevel(level: Entity) {
        // Find the required entities in the scene.
        guard let physicsRoot = level.findEntity(named: "PhysicsRoot"), let portalWorldRoot = level.findEntity(named: "Level") else {
            return
        }
        ...
        // Add portal crossing components to each descendant of the portal world root.
        portalWorldRoot.forEachDescendant { entity in
