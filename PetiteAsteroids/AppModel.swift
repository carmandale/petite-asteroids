...
class AppModel {
    ...
    let floorPortalEntity = Entity()
    let backgroundPortalEntity = Entity()
    ...
    init() {
        ...
        var portalMeshDescriptor = PortalMeshDescriptor(width: GameSettings.volumeSize.width,
                                                        ...
                                                        withBend: GameSettings.isFloorPortalBent,
                                                        bendRadius: GameSettings.portalBendRadius,
                                                        bendSegmentCount: GameSettings.portalBendSegmentCount)
        // Create the floor portal.
        floorPortalEntity.name = "FloorPortalEntity"
        floorPortalEntity.components.set(ModelComponent(mesh: generateFloorPortal(descriptor: portalMeshDescriptor), materials: [PortalMaterial()]))
        floorPortalEntity.position = [0, -GameSettings.volumeSize.height / 2, 0]
        ...
        floorPortalEntity.components.set(OpacityComponent(opacity: 0))
        root.addChild(floorPortalEntity)

        // Create the background portal.
        backgroundPortalEntity.name = "BackgroundPortalEntity"
        backgroundPortalEntity.components.set(PortalComponent(target: emptyPortalWorld))
        portalMeshDescriptor.withBend = !portalMeshDescriptor.withBend
        ...
        root.addChild(backgroundPortalEntity)
    }

    func playLevel(gameLevel: GameLevel) {
        ...
        // Set up both portals to render the level.
        ...
        // Start the intro animation for the scene.
